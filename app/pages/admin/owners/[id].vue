...
const { imagesInput, image, imagesAdd, selectedImages, removeImageFromS3 } = useImageUpload()
const { makeToast } = useToastNotification()

const { data: ownerData } = await useAsyncData('owner', () =>
  $fetch(`/api/owners/${route.params.id}`))

const owner = ref(ownerData.value.owner)

onMounted(() => {
  uploadedFiles.value = owner.value.photo
})

async function onUpdateOwner() {
  const data = new FormData()
  data.append('name', owner.value.name)
  data.append('about', owner.value.about)
  if (image.value && image.value.length) {
    const currentOwnerPhotoAwsKey = owner.value.photo.split('/').pop()
    removeImageFromS3(currentOwnerPhotoAwsKey)
    data.append('photo', selectedImages.value[0])
  }

  await $fetch(`/api/owners/${route.params.id}`, {
    method: 'PUT',
    body: data,
  })

  console.log(`The product ${owner.value.name} is updated successfully...`)
  router.push('/admin/owners')
  makeToast({ type: 'owner', title: owner.value.name, status: 'update' })
}
      ...
      <div class="row">
        ...
        <div class="col-md-6 offset-md-3">
          ...
          <b-form ref="productForm" class="d-flex flex-column gap-3">
            <!-- Owner name -->
            <b-form-group
              label="Name:" label-for="ownerName"
            ...
            <b-form-group label="Photo:" label-for="ownerPhoto">
              <client-only>
                <BFormFile id="ownerPhoto" ref="imagesInput" type="file" :multiple="false" accept="image/*" @update:model-value="imagesAdd" />
              </client-only>
            ...
            <b-row v-else-if="owner.photo !== ''" align-v="center" class="uploaded-files">
              <div class="img-wrapp p-2">
                <b-img thumbnail fluid :src="owner.photo" />
              </div>
            </b-row>

            <div class="d-flex gap-2">
              <b-button type="button" variant="primary" @click.prevent="onUpdateOwner">
                Update Owner
              </b-button>
